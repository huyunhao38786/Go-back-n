...
// Adjusts the transmission window size for fast transmission
void set_window_fast(){
	printf("set_window_fast: window_size = %d\n", WINDOW_SIZE_FAST);
	s.windowsize = WINDOW_SIZE_FAST;
}

// Adjusts the transmission window size for slow transmission
...
ssize_t gbn_send(int sockfd, const void *buf, size_t len, int flags){
	...
	// Initially set the transmission window to a larger size for faster transmission
	set_window_fast();
	// Offset to track how much data has been sent
	size_t offset = 0;
	...
	while(offset < len){
		...
		while(TRUE){
			...
			for (acked_packet = 0; acked_packet < cnt_packet; acked_packet++) {
				...
				// Check for errors in ACK reception
...
ssize_t gbn_recv(int sockfd, void *buf, size_t len, int flags){
	...
	while(1){
		...
		// If no data is received or error occurred, return the result
	...
	increment_seq_num();
	// Copy DATA packets' contents to the buffer
	if (packet.type == DATA) {
...
ssize_t recv_ack(int sockfd, gbnhdr *packet, int flags) {
	...
	alarm(TIMEOUT);
	// Try to receive a packet from the socket
	ssize_t result = recvfrom(sockfd, packet, sizeof(gbnhdr), flags, (struct sockaddr *)&s.sockaddr, &s.socklen);
	// Cancel the alarm after the attempt to receive the packet
	alarm(INT_MAX);
